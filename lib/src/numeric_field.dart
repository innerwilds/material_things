...
class NumericField extends StatefulWidget {
  ...
  final Trigger? validationTrigger;
...
class _NumericFieldState extends State<NumericField> {
  ...
  void regenerateFocusNodes([bool forceAutomatic = false]) {
    focusNodes.regenerate(
      newLength: length,
      generate: (_) => FocusNode(
        onKeyEvent: onKeyEvent,
      )..addListener(onFocused),
      onRemove: (fn) {
  ...
  void regenerateControllers([bool forceAutomatic = false]) {
    ...
    if (isAutomatic) {
      if (forceAutomatic || automaticController == null) {
        automaticController = TextEditingController()..addListener(onTextChanged);
      }
    } else {
  ...
  void unlistenExternalController([NumericField? widget]) {
    (widget ?? this.widget).controller?.removeListener(onControllerValueChanged);
  }
  ...
  void trimEnd() {
    ...
    switch (direction) {
      ...
      case TextDirection.ltr:
        for (var i = controllers.length - 1; i >= widget.initialAutomaticLength; i--) {
  ...
  void initState() {
    ...
    widget.validationTrigger?.addListener(onValidationTrigger);
  ...
  void didUpdateWidget(covariant NumericField oldWidget) {
    ...
    if (oldWidget.validationTrigger != widget.validationTrigger) {
      oldWidget.validationTrigger?.removeListener(onValidationTrigger);
      widget.validationTrigger?.addListener(onValidationTrigger);
    }
  }
  ...
  void dispose() {
    ...
    widget.validationTrigger?.removeListener(onValidationTrigger);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    assert(textStyle.fontSize != null, 'TextStyle.fontSize must not be null when using with NumericField');

    Widget buildCell(FocusNode fn, TextEditingController tec, double width, double fontSize) {
      return SizedBox(
        width: width,
        child: TextField(
      ...
      builder: (context, constraints) {
          ...
          switch (widget.automaticResizeMode) {
            ...
            case AutomaticResizeMode.findSize:
              ...
              if (calcOccupied(getWidth(fontSize, coef), spacing) > maxWidth) {
                if (widget.automaticResizeMode == AutomaticResizeMode.findSizeOrWrap) {
        ...
        final cells = [
          ...
          if (isAutomatic)
            Opacity(
              opacity: 0.5,
              child: buildCell(automaticFocusNode!, automaticController!, cellWidth, fontSize),
            ),
        ];
    ...
    // TODO(innerwilds): replace external trigger with internal after you know
    if (widget.validationTrigger != null) {
...
class NumericEditingController extends ValueNotifier<NumericEditingValue> {
  NumericEditingController([BigInt? value]) : super(
    NumericEditingValue(
      isValid: true,
  ...
  set number(BigInt? newValue) => value = value.copyWith(
    number: newValue,
  );
  set isValid(bool newValue) => value = value.copyWith(
    isValid: newValue,
  );
  set length(int newValue) => value = value.copyWith(
    length: newValue,
  );
  set focusedDigit(int newValue) => value = value.copyWith(
    focusedDigit: newValue,
  );
}
